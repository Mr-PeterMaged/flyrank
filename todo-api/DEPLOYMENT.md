...
Run npm ci and the documented database setup. Configure database, Supabase and LLM variables in .env and run npm start on a persistent Node.js host. The existing /tasks routes are unauthenticated: restrict access before using real data. /docs serves Swagger. The Vercel homepage is a product overview; API behavior is retained on other paths.
...
Existing checks: `node --check server.js`. After deployment, check desktop/mobile layout and the developer link, then exercise the real application workflow. Verify health, authentication, writes and logout where applicable. Configure the backend before expecting application data or jobs to work.
