# PDF Report Generator: deployment

## Vercel settings

| Setting | Value |
|---|---|
| Root Directory | `pdf-report-generator` |
...
Run npm ci, npx playwright install --with-deps chromium, npm run seed and npm start on a persistent Node.js host. Persist `report.db` at the project root and the `reports/` directory. Set PORT. Chromium executes on the backend. The current API has no authentication: add access control before using private records.
